...
let capitalized = [];
const upperCase = sailingClub.forEach((member) =>
	capitalized.push(member.toUpperCase())
);
...
const emails = active.map(
	(sailor) => sailor.email || `${sailor.name}@wicsail.io`
);
// emails.forEach(sailor => sendEmail(sailor))
// console.log(active)
// console.log(emails)

// Get the active sailors and return the emails of those active sailors
const activeSailors = sailors
...
const distinctColors = dogs.reduce((colors, dog) => {
	if (colors.includes(dog.color)) return colors;
	return [...colors, dog.color];
}, []);
console.log(`distinctColors are: ${JSON.stringify(distinctColors)}`);

const distinctBreeds = dogs.reduce((breeds, dog) => {
	if (breeds.includes(dog.breed)) return breeds;
	return [...breeds, dog.breed];
}, ['corgi'])
console.log(`distinctBreeds are: ${JSON.stringify(distinctBreeds)}`);

// Summing values of an object in an array with reduce()
// let initialValue = 0;
// let reducedSum = [{x: 1}, {x: 2}, {x: 3}].reduce((accumulator, currentValue) => {
//   return accumulator + currentValue.x
// }, initialValue)
// console.log(`sum of array of objects is: ${reducedSum}`)
...
); // Far more elegant than a for loop!
// console.log(total);
